...
export default function EditWhatsappGroupPage() {
    ...
    try {
      ...
      if (newImage.base64) {
        const uploadRes = await fetch("/api/upload", {
          ...
          body: JSON.stringify({ images: [newImage.base64], folder: "whatsapp_groups" }),
        });
  ...
  if (loading) return <div className="p-10 text-center">Loading group details...</div>;

  return (
    <div className="max-w-2xl mx-auto p-6 bg-card text-card-foreground rounded-xl shadow-sm mt-10 border border-border">
      <h1 className="text-2xl font-bold mb-6">Edit WhatsApp / Community Group</h1>
      <form onSubmit={handleUpdate} className="space-y-6">
        
        {/* City / Name */}
        <div className="space-y-2">
          <Label htmlFor="city">Group Name / City</Label>
          <Input
            id="city"
            required
            value={formData.city}
        ...
        {/* Invite Link */}
        <div className="space-y-2">
          <Label htmlFor="link">Invite / Join Link</Label>
          <Input
            id="link"
            type="url"
            required
